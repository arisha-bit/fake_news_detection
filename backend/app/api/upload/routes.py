"""
Upload API — POST /upload/image, POST /upload/pdf

Accepts multipart uploads, extracts text (OCR for images, PyMuPDF for PDFs),
and returns fake-news predictions using the existing pipeline.

Future routes: POST /upload/url
"""
...
async def upload_image(
    ...
    - **model**: inference model to use — `logistic` (default), `lstm`, or `bert`
    """
    logger.info(
    ...
    try:
        ...
        # 5. Run prediction via the existing service (no logic duplicated)
        ...
        # 8. Link prediction back to the uploaded file record
...
async def upload_pdf(
    ...
    """
    Upload a PDF document, extract text, and return a fake-news prediction.

    - **file**: multipart PDF upload
    - **model**: inference model to use — `logistic` (default), `lstm`, or `bert`
    """
    logger.info(
        "PDF upload started — user=%s, file=%s, model=%s",
        current_user.id,
        file.filename,
        model,
    )
    ...
    # 2. Persist file to uploads/pdfs/
    safe_filename, original_filename, file_path = await save_pdf_upload(file)

    uploaded_file_record: UploadedFile | None = None

    try:
        # 3. Persist file metadata to DB
        uploaded_file_record = UploadedFile(
            user_id=current_user.id,
            filename=safe_filename,
            original_filename=original_filename,
        ...
        logger.info("PDF extraction complete — %d chars extracted", len(extracted_text))

        # 5. Run prediction via existing pipeline
        model_choice = model.lower()
        result = _run_model(model_choice, extracted_text)
        ...
        logger.info(
            "PDF prediction completed — result=%s, confidence=%.4f",
            result["prediction"],
            result["confidence"],
        )
